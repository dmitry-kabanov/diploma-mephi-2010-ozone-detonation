...
public:
    ...
    ~Mixture();
	/**
	 * Возвращает количество компонент в смеси.
	 *
	 * @return количество компонент в смеси
	 */
	int getNSpecies() const { return nSubstances; }
    /**
    ...
    * компонентов.
	...
	/**
	 * Пишет в массив mf значения мольных долей компонент смеси.
	 *
	 * @param mf массив, в который пишутся мольные доли
	 */
	void returnMoleFractions(RealType *mf);
    /**
...
private:
	/**
	 * Возвращает номер температурного интервала, в котором находится 
	 * температура t для вещества i.
	 *
	 * @param i номер вещества
	 * @param t температура, К
	 * @return  номер температурного интервала
	 */
	int getTemperatureRangeNumber(int i, RealType t);
};
